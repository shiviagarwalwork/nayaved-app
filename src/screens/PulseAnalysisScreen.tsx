...
export default function PulseAnalysisScreen() {
  ...
  const processBrightnessData = useCallback((brightness: number, detected: boolean, timestamp: number) => {
    ...
    if (detected && brightness > 0) {
      // Add sample to PPG buffer
  ...
  const completeAnalysis = async () => {
    ...
    if (samples.length >= 60) {
      ...
      if (analysis.heartRate > 0 && analysis.quality !== 'poor') {
        // Use real data
        const values = samples.map(s => s.brightness);
      ...
      // Not enough samples - provide feedback
      Alert.alert(
        'Insufficient Data',
        'Could not detect enough pulse signal. Please ensure:\n\n• Your finger completely covers the camera and flash\n• You hold still during measurement\n• The room is not too bright\n\nWould you like to try again?',
        [
